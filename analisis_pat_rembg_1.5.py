...
for filename in os.listdir(fotos):
    if filename.endswith(".jpg") or filename.endswith(".JPG"):
        ...
        background = np.zeros((normal.shape[0], normal.shape[1], 3), dtype=np.uint8)
        ...
        media = np.mean(gray)
        #canny_inf = int(0.0*mediana) 
        #canny_sup = int(0.33*mediana)
        canny_inf = int(0*media) 
        canny_sup = int(1.5*media)
        #print (mediana)
        print (media)
        #break
        ...
        if len(contornos) > 0 and len(contornos) < 200:
            ...
            cv.imwrite(str(path+namegray), gray)  
